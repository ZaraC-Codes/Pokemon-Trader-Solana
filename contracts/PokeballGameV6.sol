 ...
 *
 * CHANGELOG v1.6.0:
 * - PYTH ENTROPY: Replaced POP VRNG with Pyth Entropy for randomness
 * - NO WHITELIST: Pyth Entropy doesn't require caller whitelisting
 * - NEW CALLBACK: entropyCallback(uint64, address, bytes32) replaces randomNumberCallback
 * - SEQUENCE NUMBERS: Use uint64 sequence numbers instead of uint256 requestIds
 * - FEE HANDLING: Entropy requests require fee via getFeeV2()
 *
    ...
    IEntropyConsumer
{
    ...
    // ============ State Variables ============
    // IMPORTANT: Storage layout must remain compatible with v1.5.x
     ...
     * @notice OpenZeppelin upgrade plugin requires gap = 38 based on storage layout analysis
     */
